...
| Mover Topológico | `clay_item_volume_move_topological` | SDF | The same drag with its reach measured **along the material** rather than through space, so a part close in space and far along the surface is left behind. It bakes **twice** — once for the verb, and once more to put the placement back feathered, which the verb strips (#128) — and both bakes take a band wide enough to carry the drag, without which the move is clamped to the band and expressed only 0.06 of it. Measured on the starting sphere at brush 0.35, that repair costs **2.6x the peak memory and 2.2x the time** of the unrepaired stroke (190 → 503 MB, 51 → 111 ms at a 0.4 pull; 405 → 2225 MB, 461 → 799 ms at 2.0), so it is the most expensive stroke on the shelf and the one to reach for when the cheap drag pulls something it should not |
...
down, **drag to draw one freehand**, drag a point to move it, **double-click
the line to add one between two others**, Del removes the selected ones. What
makes it
...
travels that far, so the same path serves both and there is no mode to be in.
Spacing in tube-widths rather than pixels, so a thick tube gets the coarse chain
it wants and a fine one keeps its detail — a point per frame at sixty hertz is a
curve nobody can edit afterwards, and going back to it is the whole difference
between this and a brush.

A freehand stroke stays on the plane its first point chose. Re-picking the
surface for each point is worse than it sounds: by the second one there is a
...
clicked. It also made the double-click below impossible: the first press had
already added a stray point before the second could be read as a double.
