...
//! against zero, because "it changed something" is what both of them did
...
//!
//! Mover Topológico was the third of them, measured here on a horseshoe whose
//! tips are 0.5 apart in space and about 1.3 apart through the bend: it lifted
//! the anchored tip 0.295 and left the far one at 0.000, where the Euclidean
//! drag carried it 0.089. The reach was honest and what carried it was not —
//! the field has no verb that drags it, so the tool baked the region into a
//! volume at 0.02 cells and replaced it, and the bake reached the sculptor as
//! a stair-stepped box (#128). The tool is gone and its fixture with it.
...
#[test]
fn the_shelf_offers_the_two_on_a_field() {
