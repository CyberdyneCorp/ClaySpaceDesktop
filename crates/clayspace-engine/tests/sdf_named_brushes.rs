...
/// spends no more than a verb which makes a single edit.
#[test]
fn one_undo_takes_the_topological_drag_back_whole() {
    let drag: Vec<GestureSample> = (0..3)
        .map(|step| {
            let t = step as f32 / 2.0;
            GestureSample {
                position: [0.0, 0.0, 1.0 + t * 0.4],
                pressure: 1.0,
                time: t,
            }
        })
    ...
    let spend = |tool: ToolKind| {
        let mut document = sphere();
        let resting = reach(&document, [0.0, 0.0, 1.0]);
        let before = SculptModel::history(&document).depth;
        document
            .apply_stroke(tool, brush, &drag, [false; 3])
            .expect("the stroke was refused");
        let spent = SculptModel::history(&document).depth - before;
        (document, resting, spent)
    };

    let (_, _, smoothing) = spend(ToolKind::Suavizar);
    let (mut document, resting, spent) = spend(ToolKind::MoverTopologico);
    let dragged = reach(&document, [0.0, 0.0, 1.0]);
    assert!(
        dragged > resting + 1e-3,
        "the drag did not land: {resting:.4} -> {dragged:.4}"
    );
    assert_eq!(
        spent, smoothing,
    ...
    let back = reach(&document, [0.0, 0.0, 1.0]);
    assert!(
        (back - resting).abs() < 5e-3,
        "one undo left the surface at {back:.4} where it rested at {resting:.4}"
    );
}
