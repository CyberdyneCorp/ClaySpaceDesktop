...
pub struct Strings {
    ...
    pub log_split: &'static str,
    pub log_prune: &'static str,
    ...
    /// What a scale drag does, said where the three factors are read.
    pub hint_axis_scale: &'static str,
    ...
    /// The pass stack on a hierarchy: the row that stands for the form under
    /// the passes, the row for a pass, and what can be done to one.
    pub multires_form: &'static str,
    pub multires_form_hint: &'static str,
    pub multires_pass_hint: &'static str,
    pub multires_add_pass: &'static str,
    pub multires_lock: &'static str,
    pub multires_unlock: &'static str,
    pub multires_locked: &'static str,
    pub multires_merge: &'static str,
    pub multires_bake: &'static str,
    pub multires_remove: &'static str,
    pub multires_mask: &'static str,
    pub multires_vertices: &'static str,
    ...
    ///
    /// A surface that moved while nobody touched the window is otherwise a
    /// defect report with no cause in it, so the status area says whether the
    /// application is listening and when an agent last acted.
    pub label_agent: &'static str,
    pub state_agent_listening: &'static str,
    pub state_agent_shut: &'static str,
    pub state_agent_connected: &'static str,
    pub state_agent_acted: &'static str,
    pub menu_agent_open: &'static str,
    pub menu_agent_close: &'static str,
    pub menu_agent_access: &'static str,
    pub title_agent_access: &'static str,
    /// Said beside the secret, because a secret on a screen is a secret in
    ...
    /// What each gate is, in the order of `AgentGate`'s own six.
    ///
    /// An array, so a gate added to the vocabulary stops this compiling until
    /// it has been worded — the same guarantee `tool_names` gives.
    pub agent_gates: [&'static str; 6],
    ...
    /// The last repair's counts: what it found, closed and left.
    pub repair_found: &'static str,
    pub repair_closed: &'static str,
    pub repair_remaining: &'static str,
    pub repair_cells_added: &'static str,
    ...
    /// What leaving a hierarchy loses: the levels under the surface, which is
    /// the hierarchy's history and not a field's parametric one.
    pub convert_levels_lost: &'static str,
...
const PT_BR: Strings = Strings {
    ...
    icon_descriptions: ["visível", "oculto", "bloqueado", "fantasma", "expandir", "recolher", "adicionar", "remover", "mover", "girar", "escalar", "unir", "subtrair", "interseção", "afunilar", "torcer", "pintar máscara", "enquadrar", "malha", "formas", "gaiola", "referência", "curva", "desfazer", "refazer", "campo de distância", "grade de voxels", "malha de triângulos", "hierarquia de subdivisão", "superfície dinâmica"],
    ...
    representation_names: ["Campo (SDF)", "Voxels", "Malha", "Hierarquia", "Dinâmica"],
    ...
    hint_axis_scale: "Uma caixa em um eixo estica esse eixo; a do centro estica os três.",
...
libera em vez de congelar.",
    ...
    multires_form: "Forma",
    multires_form_hint: "o traço entra na forma sob os passes",
    multires_pass_hint: "o traço entra neste passe",
    multires_add_pass: "Novo passe",
    multires_lock: "Travar os coeficientes",
    multires_unlock: "Destravar os coeficientes",
    multires_locked: "os coeficientes deste passe estão travados; o nome, o peso, a visibilidade e a máscara não",
    multires_merge: "Fundir no passe abaixo",
    multires_bake: "Fundir na forma",
    multires_remove: "Excluir o passe",
    multires_mask: "este passe guarda uma máscara própria",
    ...
    label_agent: "AGENTE",
    state_agent_listening: "ouvindo",
    state_agent_shut: "porta fechada",
    state_agent_connected: "conectado",
    state_agent_acted: "um agente agiu",
    menu_agent_open: "Abrir a porta do agente",
    menu_agent_close: "Fechar a porta do agente",
    menu_agent_access: "Endereço e chave do agente…",
    ...
    agent_gates: [
        ...
        "abrir um documento sobre este",
        "descartar o trabalho não salvo",
        "remover algo que desfazer não traz de volta",
        "fechar o aplicativo",
    ],
    ...
    convert_levels_lost: "os níveis de subdivisão não voltam",
    ...
    repair_found: "encontrados",
    repair_closed: "fechados",
    repair_remaining: "restantes",
    repair_cells_added: "células adicionadas",
...
const EN_US: Strings = Strings {
    ...
    icon_descriptions: ["visible", "hidden", "locked", "ghost", "expand", "collapse", "add", "remove", "move", "rotate", "scale", "union", "subtract", "intersect", "taper", "twist", "paint mask", "frame", "polyframe", "shapes", "cage", "reference", "curve", "undo", "redo", "distance field", "voxel grid", "triangle mesh", "subdivision hierarchy", "dynamic surface"],
    ...
    representation_names: ["Field (SDF)", "Voxels", "Mesh", "Hierarchy", "Dynamic"],
    ...
    hint_axis_scale: "A box on an axis stretches that axis; the centre one stretches all three.",
...
instead.",
    ...
    multires_form: "Form",
    multires_form_hint: "the stroke goes into the form under the passes",
    multires_pass_hint: "the stroke goes into this pass",
    multires_add_pass: "New pass",
    multires_lock: "Lock the coefficients",
    multires_unlock: "Unlock the coefficients",
    multires_locked: "this pass's coefficients are locked; its name, weight, visibility and mask are not",
    multires_merge: "Merge into the pass below",
    multires_bake: "Bake into the form",
    multires_remove: "Delete the pass",
    multires_mask: "this pass carries a mask of its own",
    ...
    label_agent: "AGENT",
    state_agent_listening: "listening",
    state_agent_shut: "door shut",
    state_agent_connected: "connected",
    state_agent_acted: "an agent acted",
    menu_agent_open: "Open the agent door",
    menu_agent_close: "Shut the agent door",
    menu_agent_access: "Agent address and key…",
    ...
    agent_gates: [
        ...
        "export to a file",
        "open a document over this one",
        "discard the unsaved work",
        "remove something undo cannot bring back",
    ...
    convert_levels_lost: "the subdivision levels do not come back",
    ...
    repair_cells_added: "cells added",
    action_undo: "Undo",
    action_redo: "Redo",
...
const ES_419: Strings = Strings {
    ...
    representation_names: ["Campo (SDF)", "Vóxeles", "Malla", "Jerarquía", "Dinámica"],
    ...
    hint_axis_scale: "Una caja en un eje estira ese eje; la del centro estira los tres.",
...
lados. Con Ctrl, libera en vez de congelar.",
    ...
    multires_form: "Forma",
    multires_form_hint: "el trazo entra en la forma bajo los pases",
    multires_pass_hint: "el trazo entra en este pase",
    multires_add_pass: "Nuevo pase",
    multires_lock: "Bloquear los coeficientes",
    multires_unlock: "Desbloquear los coeficientes",
    multires_locked: "los coeficientes de este pase están bloqueados; el nombre, el peso, la visibilidad y la máscara no",
    multires_merge: "Fundir en el pase de abajo",
    multires_bake: "Fundir en la forma",
    multires_remove: "Eliminar el pase",
    multires_mask: "este pase guarda una máscara propia",
    multires_vertices: "vértices",
    multires_compact: "Compactar",
    ...
    label_agent: "AGENTE",
    state_agent_listening: "escuchando",
    state_agent_shut: "puerta cerrada",
    state_agent_connected: "conectado",
    state_agent_acted: "un agente actuó",
    menu_agent_open: "Abrir la puerta del agente",
    menu_agent_close: "Cerrar la puerta del agente",
    menu_agent_access: "Dirección y clave del agente…",
    ...
    agent_gates: [
        "sobrescribir un archivo que ya existe",
        "exportar a un archivo",
        "abrir un documento sobre este",
        "descartar el trabajo no guardado",
        "quitar algo que deshacer no recupera",
    ...
    convert_levels_lost: "los niveles de subdivisión no vuelven",
    ...
    repair_found: "encontrados",
    repair_closed: "cerrados",
    repair_remaining: "restantes",
    repair_cells_added: "celdas añadidas",
...
impl Strings {
    ...
    pub fn all(&self) -> Vec<&'static str> {
        let mut all = vec![
            ...
            self.multires_peak,
            self.multires_form,
            self.multires_form_hint,
            self.multires_pass_hint,
            self.multires_add_pass,
            self.multires_lock,
            self.multires_unlock,
            self.multires_locked,
            self.multires_merge,
            self.multires_bake,
            self.multires_remove,
            self.multires_mask,
