...
pub enum ToolKind {
    ...
    /// would hide that.
    ///
    /// Its bake is placed feathered, over a band wide enough to carry the drag,
    /// and neither half is optional: the verb rebuilds the volume and throws the
    /// feather away, so the placement landed hard and rippled the normals at the
    /// cell wavelength (#128), and a feathered placement over the default band
    /// expresses the move only as far as the band. See
    /// `ClayDocument::topological_move_stroke`, which carries the measurements.
    MoverTopologico,
