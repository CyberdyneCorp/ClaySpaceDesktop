...
0.29.1. Neither 0.30.0 nor 0.39.0 has been measured on that machine, so these
are the last measured numbers rather than this pin's — and on the Linux/CUDA
box 0.39.0 halves the dab, so they are pessimistic as well as stale.
...
| `just test` | The whole suite, 609 tests, no display or GPU required |
...
```sh
just test                         # 609 tests, no display or GPU required
