...
**Status: you can open a window, sculpt on a form, and watch it re-mesh inside
the latency budget.** Three of five milestones are delivered; the interface
shell is being built now. See [docs/roadmap.md](docs/roadmap.md).

| | |
|---|---|
| Tests | 192, all headless |
| Visual captures | 76 PNGs, regenerated by `cargo test` |
| Dab latency | 31.4 ms median, 34.0 ms p95 (budget: 50 / 100) |
| Engine | ClayCore 0.26.0, pinned as a submodule |
...
Opens a window on a starting form. Orbit with the left mouse, pan with the
middle, zoom with the wheel; `1`–`4` for the view presets, `F` to frame, `M` to
cycle materials, `Esc` to quit.
...
```sh
cargo test --workspace            # 192 tests, no display or GPU required
