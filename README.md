...
| | |
|---|---|
| Tests | 687, all headless |
| Visual captures | 240 PNGs, regenerated by `cargo test` |
...
`benchmarks/baseline-macos-aarch64.json`, still reads ClayCore 0.29.1: neither
0.30.0 nor 0.39.0 has been measured on that hardware, so its figures are the
last measured ones rather than this pin's.
...
**Escultura → Deformar** carries the whole-form deformers, taper and twist, each
one undo step. A voxel layer's recorded passes — ZBrush's layers, on a grid —
are nested under it in the layer panel: record a pass, keep sculpting, and dial
...
| `just test` | The whole suite, 687 tests, no display or GPU required |
...
```sh
just test                         # 687 tests, no display or GPU required
