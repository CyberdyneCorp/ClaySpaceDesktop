...
**Status: 106 of 109 tasks.** Four of five milestones delivered and the fifth
...
| | |
|---|---|
| Tests | 473, all headless |
| Visual captures | 165 PNGs, regenerated by `cargo test` |
| Dab latency | 12.0 ms median, 13.2 ms p95 on the reference scene (budget: 50 / 100) |
| Startup to first document | 11.6 ms |
| Engine | ClayCore 0.28.0, pinned to the release tag as a submodule |
...
| `just test` | The whole suite, 473 tests, no display or GPU required |
...
```sh
just test                         # 473 tests, no display or GPU required
