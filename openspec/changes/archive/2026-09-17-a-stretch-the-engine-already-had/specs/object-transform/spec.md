...
The rule is no longer "scale is uniform", so the requirement is not named after
that any more. The node transform has always taken a factor per axis; what this
change corrects is a reading of the engine's interface, not the interface.

- FROM: `### Requirement: Scale is uniform, and the manipulator offers only that`
- TO: `### Requirement: Scale is per axis where the engine can apply it`
