...
The rule that sculpting tools are disabled on mesh layers is withdrawn, not
edited: the requirement below replaces it under a name that says what the line
now is. Stating it as a removal and an addition rather than a modification is
deliberate — a modification may not drop a scenario, and this supersession drops
"Sculpting a mesh layer is refused", which describes behaviour that no longer
exists.
