...
Withdrawn and restated below as "Scale is per axis, on a placed object and on a
whole subtool alike". The engine's layer transform has taken a factor per axis
since ClayCore ABI 0.74.0, so the scenario that said a whole
subtool scales uniformly is reversed rather than refined, and the old name — a
scale that is per axis only "where the engine can apply it" — is what let the
two readings stand side by side.
