...
than a sculptor can see, and SHALL be measured rather than assumed. The result
SHALL carry every dab the preview showed: dabs that compounded while the
pointer was down SHALL NOT be collapsed into fewer on release, and the
measurement SHALL include a gesture long enough for that to show.
...
#### Scenario: Dabs held in one spot compound on release
- **WHEN** the user dabs one spot repeatedly without releasing the pointer,
  and then releases it
- **THEN** the surface keeps the compounded smoothing the preview showed, and
  does not return to what the first dab alone had done
