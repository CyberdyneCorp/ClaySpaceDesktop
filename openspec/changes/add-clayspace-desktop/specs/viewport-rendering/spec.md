...
After an edit, the application SHALL re-mesh only the bricks the edit's influence bound marked dirty, using the engine's brick cache dirty set, and SHALL upload only the affected geometry to the renderer. It SHALL NOT re-mesh or re-upload the whole model for a local edit.

#### Scenario: A brush dab costs what it touched
- **WHEN** a single brush dab is applied to a large model
- **THEN** the bricks re-meshed are those the dab's influence bound intersects, and every other brick's geometry is left untouched in both the cache and the renderer's buffers
