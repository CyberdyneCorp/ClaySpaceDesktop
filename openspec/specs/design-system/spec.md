...
## Purpose
The visual grammar every panel is drawn from — the palette, the accent and what
it is allowed to mark, typography, spacing, iconography and the contrast floor —
so that the interface reads as one surface and never competes with the sculpt
for attention.
## Requirements
