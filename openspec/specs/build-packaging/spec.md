...
## Purpose
How the workspace is built, gated and shipped on macOS and Linux — the
acceleration features selectable at build time, the checks CI runs on every
change, the engine revision a release names, and the licensing record that goes
with it.
## Requirements
